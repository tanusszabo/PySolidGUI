...
!** grs80
      ...
      lout=1
      end subroutine init_contants


      subroutine detide(xsta,mjda,fmjda,xsun,xmon,dxtide,lflag)
...
!** UTC version by Dennis Milbert 2018june01
      ...
      implicit double precision(a-h,o-z)
      double precision, intent(inout) :: xsta(3),xsun(3),xmon(3),dxtide(3),fmjda(1)
      double precision xcorsta(3)
      double precision h20,l20,h3,l3,h2,l2
      double precision mass_ratio_sun,mass_ratio_moon
      integer, intent(inout) :: lflag(1), mjda(1)
      integer mdj
      logical leapflag
...
!      return
      end subroutine zero_vec8
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      subroutine moonxyz(mjda,fmjda,rm,lflag)
...
!** section 3.2, pg. 38-39  routine MiniMoon
      ...
      dimension rm(3)
      integer, intent(inout) :: lflag(1),mjda(1)
      double precision, intent(inout) :: fmjda(1)
      integer mdj
      logical leapflag
      save  /limitflag/                !*** leap second table limit flag
      common/limitflag/leapflag        !*** leap second table limit flag
...
!      return
      end subroutine getghar
!*******************************************************************************
      subroutine sunxyz(mjda,fmjda,rs,lflag)
...
!      return
      end subroutine lhsaaz
!-----------------------------------------------------------------------
      subroutine geoxyz(gla,glo,eht,v0)

!** convert geodetic lat, long, ellip ht. to x,y,z

      implicit double precision(a-h,o-z)
      double precision, intent(inout) :: v0(3)
      common/comgrs/a,e2
      ...
      v0(3)=z
!      return
      end subroutine geoxyz
!-----------------------------------------------------------------------
      subroutine rge(gla,glo,vvt,x,y,z)

!** given a rectangular cartesian system (x,y,z)
!** compute a geodetic h cartesian sys   (u,v,w)

      implicit double precision(a-h,o-z)
      double precision, intent(inout) :: vvt(3)
      ...
      vvt(1)=-sb*cl*x-sb*sl*y+cb*z
      vvt(2)=-   sl*x+   cl*y
      vvt(3)= cb*cl*x+cb*sl*y+sb*z
!      write(*,'(3f10.6)') ut,vt,wt
!      return
      end subroutine rge
!-----------------------------------------------------------------------
...
!** allows single number expression of time in seconds w.r.t. mjd0
      ...
      implicit double precision(a-h,o-z)  
      integer, intent(inout) :: date(3), mjda(1)
      integer mdj
      integer y
      save /mjdoff/
      common/mjdoff/mjd0
...
!      return
      end subroutine jtsciv
!***********************************************************************
      subroutine civmjd(date,daytime,mjda,fmjda)
...
!** operation confirmed against table 3.3 values on pg.34
      ...
      double precision, intent(inout) :: daytime(3), fmjda(1)
      integer, intent(inout) :: mjda(1)
      integer mdj
      integer y
      iyr=date(1)
      imo=date(2)
      ...
      fmjda(1)=fmjd
!      return
      end subroutine civmjd


      subroutine mjdciv(mjda,fmjda,date,daytime)
...
!** therefore, fractional part of julian day + 0.5 is fractional mjd
      ...
      it1=ie*30.6001d0
!**  Sum fmjd could broken with more than 1 day, the correct value
!** to sum is MOD(fmjd, 1), the decimal part of fmjd
!      idy=ib-id-it1+fmjd
      idy=ib-id-it1+DMOD(fmjd, 1.0d0)
      it2=ie/14.d0
      imo=ie-1-12*it2
      it3=(7+imo)/10.d0
      iyr=ic-4715-it3
